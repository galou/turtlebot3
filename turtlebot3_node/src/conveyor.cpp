...
using robotis::turtlebot3::Conveyor;
...
void Conveyor::add_motors()
{
  ...
  this->declare_parameter("motors.profile_acceleration_constant", 214.577);
  this->declare_parameter("motors.profile_acceleration", 0.0);
...
void Conveyor::add_sensors()
{
  ...
  this->declare_parameter("sensors.illumination", false);

  this->declare_parameter("sensors.ir", false);

  this->declare_parameter("sensors.sonar", false);
  ...
  sensors_.push_back(
    std::make_shared<sensors::BatteryState>(
      node_handle_,
      "battery_state"));

  sensors_.push_back(
    std::make_shared<sensors::Imu>(
      node_handle_,
      "imu",
      "magnetic_field",
      "imu_link"));
  ...
  sensors_.push_back(
    std::make_shared<sensors::SensorState>(
      node_handle_,
      "sensor_state",
  ...
  sensors_.push_back(std::make_shared<sensors::JointState>(node_handle_, "joint_states", "base_link"));
...
void Conveyor::add_devices()
{
  ...
  RCLCPP_INFO(this->get_logger(), "Devices added");
}
